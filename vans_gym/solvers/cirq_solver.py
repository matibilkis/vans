...
def vansatz_keras_model(wst, observable):
...
class CirqSolver:
    ...
    def run_circuit(self, list_ops):
        trajectory = list_ops#
        wst = VAnsatz(self.qubits, trajectory)
        model = vansatz_keras_model(wst, self.observable)
        w_input = tfq.convert_to_tensor([wst.circuit])
        w_output = tf.ones((1,1)) #in case of W_state we want fidelity 1.
        model.fit(x=w_input, y=w_output, batch_size=1, epochs=100,
                    verbose=0)
        ...
        probs = np.conjugate(result.final_state)*result.final_state
        return energy, probs.real
