...
energies=[]
iterations=[]
js=np.arange(0.5,2.75,0.25)
jsn = np.linspace(0.5,2.5,100)
#
for ind,bond in tqdm(enumerate(js)):
    ...
    if ind >= len(js)-2:
        ...
        nrun=0
    evaluator = Evaluator(args,loading=True, path=path,nrun_load=nrun)
    ...
    vqe_handler = VQE(n_qubits=8,noise_config={}, problem_config=problem_config,
    ...
    np.save("results/molecular/h4/data_plot/fcis",fcis)
...
#
#     np.save("results/molecular/h4/data_plot/fcisn",fcisn)

fcisn = np.load("results/molecular/h4/data_plot/fcisn.npy")

energies=np.load("results/molecular/h4/data_plot/vansenergies.npy")
fcis=np.load("results/molecular/h4/data_plot/fcis.npy")
iterations = np.load("results/molecular/h4/data_plot/iterations.npy")
...
print(fcisn)
    # ax.legend()
plt.savefig("H41.pdf",format="pdf")
