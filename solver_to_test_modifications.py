...
class GeneticSolver:
    ...
    def TFQ_model(self, symbols, lr):
        ...
        if self.noise is True:
            output = tfq.layers.Expectation(backend=cirq.DensityMatrixSimulator(noise=cirq.depolarize(self.noise_level)))(
                    ...
                    initializer=tf.keras.initializers.RandomNormal()) ###notice initialization is not very important since we re-deinfe weights later
    ...
    def prepare_circuit_insertion(self, gates_index, block_to_insert, insertion_index, SymbolsToValues):
        ...
        for ind, g in enumerate(indices):
            ...
            if ind == insertion_index:
                for gate in block_to_insert: ##assuming the gate i insert is comprised of CNOTS and u (u = [rz rx rz])
            ...
            if 0<= g < self.number_of_cnots:
                ...
                if sol.n_qubits<= g-self.number_of_cnots < 2*self.n_qubits:
                    new_symbol = "th_"+str(len(symbols))
                    symbols.append(new_symbol)
                    circuit.append(cirq.rz(sympy.Symbol(new_symbol)).on(qubit))
                if 2*sol.n_qubits<= g-self.number_of_cnots < 3*self.n_qubits:
                    new_symbol = "th_"+str(len(symbols))
                    symbols.append(new_symbol)
                    circuit.append(cirq.rx(sympy.Symbol(new_symbol)).on(qubit))
    ...
    def kill_one_unitary(self, gates_index, SymbolToValue):
        ...
        for i1, j in enumerate(gates_index):
            ...
            if checking is True:
                ...
                if reject:
                    ...
                    if (new_energy - gates_index_energy)/np.abs(gates_index_energy) <= 0.05:
